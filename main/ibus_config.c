...
esp_err_t ibus_config_init(ibus_config_t *cfg)
{
    ...
    for (int i = 0; i < (int)NUM_DEFAULTS; i++) {
        ...
        if (ret == ESP_ERR_NOT_FOUND) {
            nvs_set_u8(cfg->nvs_handle, s_defaults[i].key, s_defaults[i].default_val);
        }
    }
...
{
    for (int i = 0; i < (int)NUM_DEFAULTS; i++) {
        if (strcmp(s_defaults[i].key, key) == 0) return s_defaults[i].default_val;
    }
    return 0;
}
...
{
    if (!cfg || !cfg->initialized || !key) return find_default(key ? key : "");
    uint8_t val;
    esp_err_t ret = nvs_get_u8(cfg->nvs_handle, key, &val);
    if (ret != ESP_OK) return find_default(key);
    return val;
}
...
void ibus_config_set(ibus_config_t *cfg, const char *key, uint8_t val)
{
    ...
    nvs_set_u8(cfg->nvs_handle, key, val);
    nvs_commit(cfg->nvs_handle);
}
